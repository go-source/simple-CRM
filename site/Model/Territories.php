...
class Territories extends \FOF30\Model\DataModel
{
    ...
    {   
		...
		//get user option for clean cache
		...
		$cache->setCaching( 1 );
		$cache_account_key = 'account_territory_'.$this->getState('code', null, 'int').'_id_'.$this->getState('gscrm_territory_id', null, 'int');
	...
	*/
		...
		$filter_options_name = 'filter_options_'.$this->getState('code', null, 'int').'_id_'.$this->getState('gscrm_territory_id', null, 'int');
		//cache filter result to load into territory form (this is not the final result, that is why it is not saved into the database)
		$cache->store($filter_list, $filter_options_name, 'com_gscrm_territory');
		}	

		if ($this->getState('count_lines', null, 'int'))
		{
		//after user checked the names for the territory, we can save the account list in the relations table
	    ...
	    for ($i = 0; $i <= $count_lines; $i++ )
			{
				...
				Helper::UpdateField('accounts', $line_value, 'territory', $this->getState('gscrm_territory_id', null, 'int'));
		...
		if($this->getState('new_note', null, 'string')) 
		{
			...
			//all notes from territories are flagged type 1 = lead or cold call | QuickCreateNote($note, $type, $account, $source, $source_id)
		...
		if($this->getState('index_for_note', null, 'int'))
		{
			$cache = \JFactory::getCache('com_gscrm_territory', '');
			$cache->setCaching( 1 );
			$cache_index_Key = 'territory_index_'.$this->getState('code', null, 'int').'_id_'.$this->getState('gscrm_territory_id', null, 'int');			
			$cache->store($this->getState('index_for_note', null, 'string'), $cache_index_Key, 'com_gscrm_territory');
		}
		...
		{	
			...
			//clean list
			$cache_Key = 'territory_listed_'.$this->getState('code', null, 'int').'_id_'.$this->getState('gscrm_territory_id', null, 'int');
			$cache->remove($cache_Key, 'com_gscrm_territory');
			
			//clean index
			$cache_Key = 'territory_index_'.$this->getState('code', null, 'int').'_id_'.$this->getState('gscrm_territory_id', null, 'int');
			$cache->remove($cache_Key, 'com_gscrm_territory');
