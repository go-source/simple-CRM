...
namespace Gg\Gscrm\Site\View\Nav_contracts; 
...
$account = $model->account;
	...
	if($account < 1)
	{
		//id is zero if this is task=add; but if redirected with parserd data we neet to set it in this form add
...
if($acc_data)
	{
		if($acc_data['company'] > 0)
				{ 
					...
					//parse company name to contract on save
	...
	//parse client names to contract on save
	?><input type="hidden" name="start_save" value="2"><?php	
	
$jinput = \JFactory::getApplication()->input;
$show_bz = $jinput->get('show_bz', null, 'int');

// set navigation buttons indexes
$prev_item = Helper::PrevItem('#__gscrm_contracts', 'gscrm_contract_id', $item_id, 0);
if($prev_item < 1){$prev_item = 1;}

$next_item = Helper::NextItem('#__gscrm_contracts', 'gscrm_contract_id', $item_id, 0);
...
<div class="btn-group col-sm-12" role="group" aria-label="...">
	...
	<a role="button" class="btn" title="<?php echo JText::_('GS_TIP_PREV') ?>" href="index.php?option=com_gscrm&view=Contract&id=<?php echo $prev_item ?>
		&Itemid=<?php echo $menu_item['contract'] ?>&<?php echo JSession::getFormToken() ?>=1" >			
	...
	<a role="button" class="btn" title="<?php echo JText::_('GS_TIP_NEXT') ?>" href="index.php?option=com_gscrm&view=Contract&id=<?php echo $next_item ?>
		&Itemid=<?php echo $menu_item['contract'] ?>&<?php echo JSession::getFormToken() ?>=1">
...
<div class="col-sm-12 mt2">	
		...
		<a class="btn-xs" href="index.php?option=com_gscrm&view=Contract&Itemid=<?php echo $menu_item['contract'] ?>&<?php echo JSession::getFormToken() ?>=1" 
		...
		<a class="btn-xs" href="index.php?option=com_gscrm&view=Order&account=<?php echo $account ?>&contract=<?php echo $item_id ?>&Itemid=<?php echo $menu_item['order'] ?>&<?php echo JSession::getFormToken() ?>=1" 
		...
		<a class="btn-xs" href="index.php?option=com_gscrm&view=Invoice&account=<?php echo $account ?>&contract=<?php echo $item_id ?>&Itemid=<?php echo $menu_item['invoice'] ?>&<?php echo JSession::getFormToken() ?>=1" 
...
<div class="col-sm-12 mt2">
		...
		<span class="ml2"> <i class="glyphicon glyphicon-briefcase"></i><?php echo ' '.JText::_('COM_GSCRM_CONTRACT_NUMBER').' '.$model->number ?></span>
...
<?php if($acc_data) { ?>
	...
	<div class="col-sm-2" >
		...
		<?php
			//hide button if this id=0, must save before switching to business information
		if ( $item_id > 0){ ?>
		<a role="button" class="btn-xs" href="index.php?option=com_gscrm&view=Contract&id=<?php echo $item_id ?>&show_bz=1&Itemid=<?php echo $menu_item['contract'] ?>&<?php echo JSession::getFormToken() ?>=1" 
		...
		<a role="button" class="btn-xs" href="index.php?option=com_gscrm&view=Contract&id=<?php echo $item_id ?>&show_bz=0&Itemid=<?php echo $menu_item['contract'] ?>&<?php echo JSession::getFormToken() ?>=1" 
	...
	<div class="col-sm-10 " >
		...
		<div class="col-sm-12" >
			<p>	<span><?php echo $acc_data['title'].'<br>' ?></span>
				<span><?php if($rel_comp['gscrm_account_id'] !== 0){ 
							echo '<a href="index.php?option=com_gscrm&view=Account&id='.$acc_data['company'].'&Itemid='.$menu_item['account'].'&'.JSession::getFormToken().'=1 " >'.$rel_comp['title'].'</a><br>';}  ?>
							</span>		
		...
		<div class="col-sm-12" >
			<p>	
				<span><?php echo $rel_comp['title'].'<br>' ?></span>
				<span><?php echo JText::_('GS_UID').': '.$rel_comp['unique_id'].'<br>' ?></span>
				<span><?php echo $address[0]['number'].' '.$address[0]['street'].'<br>' ?></span>
				<span><?php echo $rel_comp['additional'].'<br>' ?></span>
				<span><?php echo $address[0]['city'].' '.$address[0]['state'].'<br>' ?></span>
				<span><?php echo $address[0]['ZIP'].' '.$address[0]['country'].'<br>' ?></span>
