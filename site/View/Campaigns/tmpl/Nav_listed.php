...
namespace Gs\Gscrm\Site\View\Nav_listed;

//navigate through accounts listed in a campaign
...
//set navigation id 
$item_id = (int)$model->gscrm_campaign_id;
$index = 0;

//skip if compaign has not been created
if($item_id == 0){ ?><div><p><?php echo JText::_('COM_GSCRM_CAMPAIGNS_NAV_EMPTY') ?></p></div><?php ;} 	
else {
...
$cache_index_Key = 'campaign_index_'.$model->code.'_id_'.$item_id;
$index = $cache->get($cache_index_Key, 'com_gscrm_campaign');

//get list 
$cache_listed_key = 'campaign_listed_'.$model->code.'_id_'.$item_id;
$campaign = array();
$campaign = $cache->get($cache_listed_key, 'com_gscrm_campaign');

//get accounts
$cache_account_key = 'account_campaign_'.$model->code.'_id_'.$item_id;
...
if ( empty($accounts_cached)) {
	$select_array = array('title','gscrm_account_id', 'type', 'gender', 'email', 'phone1','campaign');
...
if ( empty($campaign)) {
	...
	{
		if( $account_ref->campaign == $item_id){ 
			$campaign[] = $account_ref->gscrm_account_id;
			}
	}
	$cache->store($campaign, $cache_listed_key, 'com_gscrm_campaign');
	}

//set navigation indexes
$index_max = count($campaign)-1;

//skip if a list has not been saved
if( !$campaign ){ ?><div><p><?php echo JText::_('COM_GSCRM_CAMPAIGNS_NAV_EMPTY') ?></p></div><?php ;} 	
else {

//get account index from URL and set current account to show
$jinput = \JFactory::getApplication()->input;
if ($jinput->get('index', null, 'int')) { $index = $jinput->get('index', null, 'int'); } //else{ $index = 0; }

if( $index > 1 ) { $prev_item = $index - 1; }else{ $prev_item = 0; }
if( $index < $index_max ){ $next_item = $index + 1; }else{ $next_item = $index_max; }

$account = $campaign[$index];
...
<a role="button" class="btn col-sm-2" title="<?php echo JText::_('GS_TIP_PREV') ?>"
		<?php 	if ($index > 0) { ?>href="index.php?option=com_gscrm&view=Campaign&id=<?php echo $item_id; ?>&Itemid=<?php echo $menu_item['campaign'] ?>&index=<?php echo $prev_item ?>&<?php echo JSession::getFormToken() ?>=1" >
...
<a role="button" class="btn col-sm-2" title="<?php echo JText::_('GS_TIP_NEXT') ?>"
		<?php  if ($index < $index_max) { ?> href="index.php?option=com_gscrm&view=Campaign&id=<?php echo $item_id; ?>&Itemid=<?php echo $menu_item['campaign'] ?>&index=<?php echo $next_item ?>&<?php echo JSession::getFormToken() ?>=1" >
